    ...
    <div :class="['zp-date-wrapper', `zp-date-${uid}`]" ref="dpWrap" :style="wrapStyle">
      <transition :name="`zp-${position}`">
        <div v-show="expand" class="zp-date-items" @mouseenter="dpVisible = true" @mouseleave="onBlur(false)">
          <keep-alive>
            <component :is="getComponent" v-model="curDate" @change="handleDateChange" :type="type" :format="format" :style="pickerStyle" :disabledDate="disabledDate" :withToday="withToday" :view="view" ref="dpComp" />
          </keep-alive>
        </div>
      </transition>
    </div>
...
<script>
  ...
  export default {
    ...
    props: {
      ...
      value: [Date, Array, Number, String],
      placeholder: String,
    ...
    computed: {
      ...
      pickerStyle () {
        ...
        style.width = this.isRange ? `${2 * width}px` : `${width}px`
        return style
      },
      view () {
        if (['year', 'month', 'calendar', 'date'].indexOf(this.type) > -1) {
          this.curType = 'calendar'
        }
        return this.type
      }
    },
    created () {
      if (this.isRange && Array.isArray(this.value) && this.value.length > 1) {
        this.startDate = formatDate(this.value[0])
        if (this.value.length > 1) {
          this.endDate = formatDate(this.value[1])
        }
      }
    },
    mounted () {
    ...
    methods: {
      getComponent () {
        const name = this.curType.charAt(0).toUpperCase() + this.curType.slice(1)
      ...
      clearDate () {
        this.startDate = this.endDate = this.model = ''
        if (this.isRange && this.$refs.dpComp.$refs.zpRange) {
          this.curDate = []
          this.$refs.dpComp.resetDayClasses().init()
        } else {
          this.curDate = null
        }
        this.expand = false
