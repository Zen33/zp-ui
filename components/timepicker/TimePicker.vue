<!-- Time Picker -->
<template>
  <div :class="['zp-tp', {'zp-picker-disabled': disabled}]" @mousewheel="expand = false" @click="toggleList" @blur="onBlur" @mouseleave="onBlur" ref="tpSelf">
    <input type="text" v-model="model" readonly :placeholder="placeholder" ref="tpInput" />
    <i class="zp-icon-calendar"></i>
    <i v-if="clearable" :class="['zp-icon-clear', {'zp-hide': !value || disabled}]" @click.stop="clearDate"></i>
    <div :class="['zp-time-wrapper', `zp-time-${uid}`]" ref="tpWrap" :style="wrapStyle">
      <transition :name="`zp-${position}`">
        <div v-show="expand" class="zp-time-items" @mouseenter="tpVisible = true" @mouseleave="onBlur(false)">
          <zp-time
            v-model="value"
...
<script>
  ...
  export default {
    ...
    beforeDestroy () {
      const tp = document.querySelector(`.zp-time-${this.uid}`)
    ...
    methods: {
      ...
      adjustPos (item) { // 纠正位置
        ...
        setTimeout(() => {
          this.expand = true
          this.$refs.tpComp.init().updateScroller()
        }, 0)
      },
      toggleList () { // 切换当前项
        if (this.disabled) {
      ...
      onBlur (status) { // 隐藏当前项
        status === false && (this.tpVisible = false)
        setTimeout(() => {
          if (!this.tpVisible) {
            this.expand = false
            this.$emit('tpInput:blur')
          }
        }, 300)
      }
    }
